...
describe("formatLastSeen", () => {
  ...
  it("falls back gracefully on missing/invalid input", () => {
    expect(formatLastSeen(null, NOW)).toBe("há um bom tempo");
    expect(formatLastSeen("nonsense", NOW)).toBe("há um bom tempo");
  });
});
...
describe("presenceLabel", () => {
  it("labels each state for the tooltip", () => {
    ...
    expect(presenceLabel("away", ago(1_000), NOW)).toBe("Ausente — sem atividade");
