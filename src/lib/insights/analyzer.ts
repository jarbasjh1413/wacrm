...
function buildRadarPrompt(
  ...
  return [
    ...
    '  NO QUADRO DE VENDAS: novo (mandou a primeira mensagem, ainda não sabemos nada), qualificando (estamos descobrindo se é lead de verdade — pode ser curiosidade, engano ou alguém só pesquisando), qualificado (é lead de verdade: sabemos o que procura e ele demonstrou interesse real), orcamento (já passamos preço ou proposta), negociando (ele está discutindo valor, prazo ou condição de pagamento), reservado (pediu para separar/apartar), ganho (comprou/pagou), perdido (desistiu ou disse não).',
    '  NO QUADRO DE SERVIÇO: novo (falou que tem um problema, sem detalhe), qualificado (já sabemos o equipamento E o defeito), orcamento (já passamos faixa de preço, taxa de diagnóstico ou prazo), reservado (combinou CONCRETO de trazer na loja ou aceitar a coleta — tem dia, período ou "podem buscar"; "depois eu levo" NÃO é combinado), perdido (desistiu, achou caro, sumiu ou vai em outro lugar).',
    '  NUNCA use "ganho" nem "negociando" no quadro de serviço: depois que a máquina chega quem movimenta o card é o sistema de ordem de serviço, não a conversa. Seu limite ali é "reservado".',
