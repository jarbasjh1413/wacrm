...
export function AiPlayground({ onGoToSetup }: { onGoToSetup?: () => void }) {
  const tr = useTranslations('AiPlayground');
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          toast.error(tr('notConfiguredToast'));
        } else {
          toast.error(data.error ?? tr('noReplyToast'));
        }
      ...
      toast.error(tr('unreachableToast'));
      setTurns(turns);
      setInput(text);
    } finally {
      setSending(false);
    ...
    <div className="flex h-[60vh] min-h-[420px] flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">{tr('headerTitle')}</span>
        ...
        {turns.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
            ...
            {onGoToSetup && (
              ...
              >
                {tr('goToSetup')} <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            )}
          </div>
        )}
