...
| 03/08/2026 | **Socket zumbi na Evolution (diagnóstico + remédio).** Teste do script "Catalogo" falhou com "Evolution API error: Connection Closed" em todos os envios, mas `connectionState` dizia `open` — sessão Baileys estava zumbi. Remédio: `POST /instance/restart/{name}` (não desloga, não pede QR; reconectou em segundos). | **Backlog (anotar p/ implementar):** detecção automática de socket zumbi — N falhas "Connection Closed" seguidas num intervalo → auto-restart da instância via Evolution API + notificação no CRM (§11.7 hoje só cobre connection.update, que não dispara nesse cenário). Jarbas: redisparar o script Catalogo p/ confirmar. |
