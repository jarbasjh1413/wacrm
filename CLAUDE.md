...
**REGRA DE PRODUTO — editável por padrão** (pedido do Jarbas, 08/08/2026:
"adotar em todo o projeto coisas que seriam boas de o usuário poder editar"):
todo dado que a IA escreve deve ser EDITÁVEL pelo humano na MESMA tela onde
aparece — clique para editar, Enter salva, Esc cancela. E a edição humana
trava contra a IA (source='human' / *_locked_at / *_fixada_em conforme o
caso). Toda feature nova nasce cumprindo isso; se um dado da IA aparecer em
tela sem edição, é dívida, não escolha. Já cumprem: valor/coluna/título do
negócio, ficha do cliente, etiquetas, temperatura/intenção (card do Radar).
...
| 10/08/2026 | **TRANSCRIÇÃO LIGADA + dois vazamentos de mensagens achados e consertados.** Jarbas colou a chave da OpenAI na tela → credencial `crm-transcricao` criada na Evolution, `speechToText: true` confirmado. No 'verifica' apareceram DOIS problemas: (1) **túnel cloudflared morto desde 05/08** — o CRM não recebia NADA; religado (novo URL trycloudflare, .env.local atualizado, webhook/set reapontado, teste HTTP 405 ok). Trocar de túnel a cada queda é insustentável → mais um argumento para o deploy. (2) **BUG LID**: o WhatsApp vem migrando contatos para remoteJid `...@lid` (número oculto) e o webhook só aceitava `@s.whatsapp.net` — mensagens desses clientes eram DESCARTADAS em silêncio (o áudio da Isadora de 15:05 foi jogado fora). O número real vem em `key.remoteJidAlt`; o webhook agora traduz LID→JID de telefone e segue normal (LID sem alt: pula com warn). **Descoberta de bônus**: a Evolution guarda 73 MIL mensagens no postgres dela (DATABASE_SAVE_DATA ligado) — o importador de histórico do roadmap é viável SEM re-parear, via /chat/findMessages. | Jarbas mandar/receber um áudio de teste para validar o circuito da transcrição de ponta a ponta. Avaliar backfill das mensagens perdidas (05→10/08) a partir do store da Evolution. |
| 09/08/2026 | **VPS LIGADA PARA TRANSCRIÇÃO.** Com o Jarbas logado no hPanel, pilotei o Chrome dele: Gerenciador Docker → projeto evolution-api-x49e → Editor .yaml → adicionada `OPENAI_ENABLED: true` no bloco environment do serviço api (linha 16, depois de AUTHENTICATION_EXPOSE_IN_FETCH_INSTANCES) → Implantar. Deploy ok, WhatsApp reconectou sozinho (crm-b7594faf · open) e GET /openai/creds passou de 'Openai is disabled' para `[]` — módulo ativo, aguardando credencial. NÃO digitei senha nenhuma (o login no hPanel foi dele) e nenhuma chave envolvida nesta etapa. | **Falta SÓ o Jarbas:** criar a chave em platform.openai.com (conta + cartão, ~R$5/mês) e colar em Settings → WhatsApp → Transcrição de áudio → Ligar. A partir daí todo áudio de cliente chega transcrito e o Radar passa a ouvir. |
| 09/08/2026 | **TRANSCRIÇÃO DE ÁUDIO — lado do CRM 100% pronto (055).** Item 1 da pesquisa multimídia (speechToText nativo da Evolution, Whisper/OpenAI por baixo, ~R$5/mês). (a) Migration 055: `ai_configs.transcricao_api_key` (AES-256-GCM, mesmo contrato da chave Anthropic — nunca volta ao client) + `transcricao_enabled`. (b) `configureSpeechToText` no evolution-api.ts: acha/cria a credencial 'crm-transcricao' e liga o speechToText por instância; devolve erro legível em vez de lançar. (c) Rota `/api/ai/transcricao` (GET/POST/DELETE, admin+, rate-limited): POST valida sk-, aplica nas instâncias ANTES de guardar a chave (se nenhuma aceitar, não guarda), DELETE desliga e apaga. (d) Webhook: `message.speechToText` do áudio vira content_text — o áudio ganha texto na conversa. (e) **`buildConversationContext` filtrava content_type='text': áudio era INVISÍVEL para a IA mesmo transcrito** — agora inclui áudio com texto, prefixado '[áudio transcrito]' (Radar, follow-up e auto-reply passam a 'ouvir'). Teste novo cobrindo. (f) Card 'Transcrição de áudio' em Settings→WhatsApp: o DONO cola a chave (password field), explica custo e privacidade. 745 testes, build OK, migration aplicada. **Descoberta na VPS**: o módulo OpenAI da Evolution está DESLIGADO (GET /openai/creds → 'Openai is disabled') — precisa OPENAI_ENABLED=true no env do Docker + restart. | **RETOMAR**: fui fazer isso pelo Chrome dele (autorizado), mas a sessão do hPanel expirou na tela de login — SENHA É DELE, parei ali e deixei a aba aberta (hpanel.hostinger.com). Quando ele logar: VPS srv1870897 → Docker → projeto evolution → editar env → OPENAI_ENABLED=true → restart (derruba o WhatsApp por segundos, reconecta sozinho) → conferir GET /openai/creds ≠ 400. Depois ele cria a chave em platform.openai.com e cola em Settings→WhatsApp→Transcrição. Duas pendências antigas dele: chave de API do CRM p/ o sistema de OS + eventos. |
| 08/08/2026 | **Lista "Leads de conserto sem card"** — a rede de segurança contra o erro silencioso do funil de serviço. Componente novo `leads-sem-card.tsx`: faixa âmbar acima do board (só no funil de serviço) listando conversas dos últimos 15 dias com intenção assistencia/pos_venda cujo contato NÃO tem card aberto naquele funil, cada uma com botão "Criar card" (nasce na primeira coluna, título nome — interesse). Sem ela, a IA devolvendo funil nulo deixaria lead sumir por semanas sem sintoma. Hoje a lista está vazia no banco real (nenhuma conversa de conserto pendente — correto). Lint do React Compiler exigiu o disable padrão do projeto no useEffect. 744 testes, build OK. | Próximo da fila combinada: transcrição de áudio (Evolution speechToText, ~R$4,60/mês) — ATENÇÃO: exige chave da OpenAI (Whisper), que o Jarbas ainda não tem; pedir a ele antes de começar. |
| 08/08/2026 | **Criar negócio manualmente pela lateral.** Pedido do Jarbas: a seção Negócios só mostrava, não criava. Agora o + no cabeçalho abre painel compacto: escolha do funil (vendas/serviço — só os que existem na conta), título (pré-preenchido com o nome do contato) e valor opcional. Nasce na primeira coluna do funil MAIS ANTIGO do tipo (o mesmo que o Radar usa) com conversation_id da conversa aberta. **Valor digitado à mão já nasce com value_locked_at** (a IA não mexe); sem valor, ela fica livre para preencher quando ouvir na conversa. Radar ADOTA o card manual: a busca dele por contato+funil encontra o negócio existente e passa a movê-lo/completá-lo em vez de duplicar. 744 testes, build OK. | Jarbas testar o + em Negócios. |
| 08/08/2026 | **Ficha padrão em TODO contato.** Relato do Jarbas: a Ficha do cliente só aparecia no South Import's. Causa: a lateral buscava só contact_custom_values PREENCHIDOS e escondia a seção quando vazia — contato sem dedução da IA ficava sem ficha nenhuma. Agora busca as DEFINIÇÕES (custom_fields) e cruza com os valores: todo contato mostra todos os campos da conta, os vazios com 'preencher…' em itálico, clicáveis (o upsert por contact_id+custom_field_id já criava a linha). A seção só some se a conta não tiver campo definido. 744 testes, build OK. | Jarbas conferir num contato qualquer. |
| 08/08/2026 | **Bug do título + ficha editável + REGRA "editável por padrão".** (a) Print do Jarbas: a edição do título usava input de UMA linha — título longo aparecia só pelo rabo. Virou textarea de 3 linhas com cursor no fim; Enter salva, Esc cancela. (b) **Ficha do cliente editável**: clicar no valor (Cidade, Equipamento...) edita ali mesmo; grava com source='human' e o analisador JÁ pulava source='human' (948), então a proteção era só questão de expor a tela — a consulta ganhou custom_field_id para o upsert. Tooltip: "o que você escrever, a IA não sobrescreve". (c) Pedido dele virou REGRA DE PRODUTO no §12: todo dado que a IA escreve deve ser editável na mesma tela, com trava contra a IA — dado de IA sem edição é dívida, não escolha. 744 testes, build OK. | Jarbas conferir: editar o título do negócio (agora aparece inteiro) e clicar num campo da ficha. |
| 08/08/2026 | **EDIÇÃO HUMANA QUE A IA LÊ + x nas etiquetas do Radar.** Dois pedidos do Jarbas. (a) **Título do negócio editável** na lateral (clique no título, Enter salva) — a rota quick aceita `title`; sem trava porque o Radar só escreve título na CRIAÇÃO. (b) **A IA agora LÊ o que o dono editou**: `buildDealBlock` no analyzer injeta no prompt os negócios abertos do contato (quadro, título, valor, coluna e QUAIS campos estão travados à mão) com a instrução 'trate como VERDADE, nunca contradiga campo travado' — fecha o ciclo pedido: 'seria bom pra própria inteligência ver o que eu estou editando, interpretar e salvar'. (c) **x nas etiquetas do Radar** (quente/morno/frio/intenções): clicar NÃO apaga só a etiqueta — chama POST /api/radar/{conv}/classify com valor 'indefinido', que remove a etiqueta, CONGELA o campo (a IA não recoloca) e grava a correção como exemplo no prompt das próximas análises. Toast explica: 'O Radar registrou a correção e não vai recolocá-la'. Etiqueta comum continua com x direto no contact_tags. E2E validado: reanálise da conversa do Eduardo com o bloco do negócio no prompt → erros=0. 744 testes, build OK. | Jarbas testar: clicar no título do negócio e editar; passar o mouse no 'morno' e clicar no x. Fila: lista 'Leads de conserto sem card', transcrição de áudio; do lado dele a chave de API + eventos da OS. |
| 08/08/2026 | **ETIQUETAS MANUAIS NA LATERAL** 🏷️ (relato do Jarbas: "manualmente não consigo adicionar etiquetas no lead" — a seção era só vitrine). Agora: botão **+** abre o seletor com todas as etiquetas da conta (chips clicáveis), **x** no hover tira a etiqueta, e dá para **criar etiqueta nova ali mesmo** (Enter cria e já aplica; cor sorteada de uma paleta fixa). As 8 etiquetas do RADAR (quente/morno/frio + intenções) ficam FORA do seletor manual e sem x, com tooltip explicando: mexer nelas à mão seria desfeito na próxima análise — o caminho certo é corrigir no card do Radar, que também ensina a IA. RLS conferida: contact_tags exige agent, tags exige admin (ele é owner). 7ª consulta paralela no fetch da lateral. 744 testes, build OK. **Também respondido a ele:** mover o negócio de coluna MANUALMENTE por dentro da conversa JÁ existe (o seletor de estágio no card do negócio da lateral, 051) e a temperatura manual JÁ existe (corrigir no card do Radar, 047) — faltava ser óbvio, não faltava existir. | Jarbas testar: abrir uma conversa → seção Etiquetas → botão +. Fila: lista "Leads de conserto sem card", transcrição de áudio, e do lado dele a chave de API + eventos da OS. |
| 08/08/2026 | **A TELA ALCANÇOU O MOTOR** 🎛️ (a) **Temperatura no card do board**: bolinha colorida (vermelho=quente, âmbar=morno, azul=frio) ao lado do título, carregada de conversation_insights numa consulta só por board; temperatura NÃO virou coluna de propósito — lead pode estar em Negociando e frio, em Qualificando e quente. Card também ganhou o crachá `OS #7202` lido do espelho. (b) **Interruptor do modo ensaio** em Settings→Radar, com aviso âmbar explicando o que muda ligado/desligado. (c) **Botão "↷ Virou venda / Virou conserto"** no card do negócio na lateral: rota nova POST /api/deals/[id]/converter — cria o card no outro funil amarrado por relacionado_deal_id e fecha a origem com motivo_perda (virou_venda/virou_servico/funil_errado); card em coluna dirigida pela OS NUNCA é fechado (máquina segue na bancada), só amarra. Crachá venda/conserto em cada negócio da lateral. (d) **Seletor de funis** mostra o tipo e "(o Radar usa este)"; diálogo de novo funil escolhe o tipo e semeia em português JÁ mapeado (matou o funil que nascia em inglês e mudo). (e) **Guarda no apagar funil**: contagem de negócios antes do DELETE — apagar funil com card era CASCADE silencioso. (f) **lead_frio ACORDADO**: filtrava status='active' (não existe no CHECK — morto desde o template); agora 'open' + filtro pipelines.tipo='vendas' no MESMO commit (sem o filtro cobraria venda de quem tem máquina na bancada). Seguro porque lead_frio não é AUTO e o modo ensaio está ligado. 744 testes, build OK. | Jarbas conferir no navegador: bolinha no board, interruptor em Settings→Radar, botão Virou venda na lateral, selo no seletor de funis. **Segue pendente dele:** gerar a chave de API e ligar o sistema de OS (docs/integracao-os.md). Próximo da fila: lista "Leads de conserto sem card" no board de serviço + transcrição de áudio (~R$4,60/mês). |
| 08/08/2026 | **MODO ENSAIO + os dois funis conforme a operação real (054).** Três pedidos do Jarbas. **(1) "Não quero nada já cobrando clientes agora"** — descoberto que `equipamento_pronto` e `pos_venda` enviavam SOZINHOS (AUTO_CENARIOS): se a integração da OS ligasse, a primeira máquina parada há 3 dias já receberia WhatsApp. Criado `followup_settings.modo_ensaio` — o agente detecta, escreve e enfileira, mas **nada sai para o cliente**; tudo cai na fila de aprovação. LIGADO na conta dele, e o DEFAULT_SETTINGS do código também nasce em ensaio (conta sem config não manda mensagem sozinha). **(2) "Conforme a OS é movimentada, movimentar no funil do CRM"** — o funil de serviço ganhou 4 colunas dirigidas pela OS via `pipeline_stages.os_situacao` (na_bancada, aguardando_cliente, pronto, finalizada). NÃO são as 12 do kanban dele: são as que mudam o que o CRM tem a DIZER ao cliente. Dois motores sem colisão — a IA move por `radar_stage`, a OS move por `os_situacao`, e as colunas da OS têm radar_stage NULO, então a IA não as alcança nem querendo. A ponte só anda para frente (OS que volta de coluna não puxa card) e respeita card arrastado à mão. **(3) "Entre mandar mensagem e ser lead qualificado a gente precisa qualificar"** — vocabulário da IA passou de 6 para 8 palavras (+`qualificando`, +`orcamento`) e o funil de vendas ganhou a coluna **Qualificando**; de quebra some o bug de 'Orçamento enviado' e 'Negociando' terem a MESMA palavra (a IA nunca conseguia usar 'Negociando'). Nenhum negócio mudou de coluna. 744 testes, typecheck/lint/build OK. **Prova no banco real — a jornada inteira de um conserto**: Radar cria em "Vai trazer" → OS "Aguardando recebimento" fica → "Orçamento a fazer" vai pra Máquina na loja → "Aguardando cliente" → "Aguardando retirada" → "Finalizada" = Entregue/won. | **Falta:** temperatura (quente/morno/frio) visível no card do board — hoje ela é etiqueta e dossiê, e o Jarbas quer ver no funil; interruptor do modo ensaio na tela (hoje só no banco); botões "Recebi a máquina" e "Virou venda/serviço"; guarda de contagem no `handleDeletePipeline`; cenário `lead_frio` morto. **Do lado dele:** gerar a chave de API e pedir ao sistema de OS para dar POST em /api/v1/os-events. Com o modo ensaio ligado dá para ligar a integração SEM risco de mensagem sair. |
